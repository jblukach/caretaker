...
class CaretakerVirtualFabric(Stack):
    ...
    ### LAMBDA ###
        ...
        virtualfabric = _lambda.Function(
            self, 'virtualfabric',
            runtime = _lambda.Runtime.PYTHON_3_12,
            architecture = _lambda.Architecture.ARM_64,
            code = _lambda.Code.from_asset('sources/ip/virtualfabric'),
            timeout = Duration.seconds(900),
            handler = 'virtualfabric.handler',
            environment = dict(
                AWS_ACCOUNT = account,
                FEED_TABLE = 'feed',
                VERIFY_TABLE = 'verify',
        ...
        parameter = _ssm.StringParameter(
            ...
            parameter_name = '/caretaker/ip/virtualfabric',
            string_value = virtualfabric.function_name,
            tier = _ssm.ParameterTier.STANDARD
        )

        logs = _logs.LogGroup(
            self, 'logs',
            log_group_name = '/aws/lambda/'+virtualfabric.function_name,
        ...
        event.add_target(
            _targets.LambdaFunction(virtualfabric)
        )

    ### LAMBDA ###

        virtualfabricdomain = _lambda.Function(
            self, 'virtualfabricdomain',
            runtime = _lambda.Runtime.PYTHON_3_12,
            architecture = _lambda.Architecture.ARM_64,
            code = _lambda.Code.from_asset('sources/dns/virtualfabric'),
            timeout = Duration.seconds(900),
            handler = 'virtualfabric.handler',
            environment = dict(
                AWS_ACCOUNT = account,
        ...
        parameter = _ssm.StringParameter(
            self, 'parameter2',
            description = 'Project Caretaker',
            parameter_name = '/caretaker/dns/virtualfabric',
            string_value = virtualfabricdomain.function_name,
            tier = _ssm.ParameterTier.STANDARD
        )

        virtualfabricdomainlogs = _logs.LogGroup(
            self, 'virtualfabricdomainlogs',
            log_group_name = '/aws/lambda/'+virtualfabricdomain.function_name,
            retention = _logs.RetentionDays.ONE_MONTH,
            removal_policy = RemovalPolicy.DESTROY
        )
        ...
        virtualfabricdomainevent.add_target(
            _targets.LambdaFunction(virtualfabricdomain)
        )
