...
class CaretakerUltimateHosts(Stack):
    ...
    ### LAMBDA ###
        ...
        parameter = _ssm.StringParameter(
            ...
            parameter_name = '/caretaker/ip/ultimatehosts',
            string_value = ultimatehosts.function_name,
    ...
    ### LAMBDA ###
        ...
        parameter = _ssm.StringParameter(
            self, 'parameter2',
            description = 'Project Caretaker',
            parameter_name = '/caretaker/dns/ultimatehosts',
            string_value = ultimatehostsdns.function_name,
            tier = _ssm.ParameterTier.STANDARD
        )
        ...
        ultimatehostsdnssub = _logs.SubscriptionFilter(
            self, 'ultimatehostsdnssub',
            log_group = ultimatehostsdnslogs,
            destination = _destinations.LambdaDestination(error),
            filter_pattern = _logs.FilterPattern.all_terms('ERROR')
        )

        ultimatehostsdnstime = _logs.SubscriptionFilter(
            self, 'ultimatehostsdnstime',
            log_group = ultimatehostsdnslogs,
            destination = _destinations.LambdaDestination(timeout),
            filter_pattern = _logs.FilterPattern.all_terms('Task','timed','out')
        )
