...
class CaretakerFeedwalla(Stack):
    ...
    ### LAMBDA ###
        ...
        feedwalla = _lambda.Function(
            self, 'feedwalla',
            runtime = _lambda.Runtime.PYTHON_3_12,
            architecture = _lambda.Architecture.ARM_64,
            code = _lambda.Code.from_asset('sources/ip/feedwalla'),
            timeout = Duration.seconds(900),
            handler = 'feedwalla.handler',
            environment = dict(
                AWS_ACCOUNT = account,
                FEED_TABLE = 'feed',
                VERIFY_TABLE = 'verify',
        ...
        logs = _logs.LogGroup(
            self, 'logs',
            log_group_name = '/aws/lambda/'+feedwalla.function_name,
            retention = _logs.RetentionDays.ONE_DAY,
            removal_policy = RemovalPolicy.DESTROY
        )

        feedwallaalarm = _cloudwatch.Alarm(
            self, 'feedwallaalarm',
            comparison_operator = _cloudwatch.ComparisonOperator.GREATER_THAN_THRESHOLD,
            threshold = 0,
            evaluation_periods = 1,
            metric = feedwalla.metric_errors(
                period = Duration.minutes(1)
            )
        )

        feedwallaalarm.add_alarm_action(
        ...
        event.add_target(
            _targets.LambdaFunction(feedwalla)
        )