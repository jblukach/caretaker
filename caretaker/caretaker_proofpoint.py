...
class CaretakerProofPoint(Stack):
    ...
    ### LAMBDA ###
        ...
        logs = _logs.LogGroup(
            self, 'logs',
            log_group_name = '/aws/lambda/'+proofpoint.function_name,
            retention = _logs.RetentionDays.ONE_MONTH,
            removal_policy = RemovalPolicy.DESTROY
        )

        proofpointalarm = _cloudwatch.Alarm(
            self, 'proofpointalarm',
            comparison_operator = _cloudwatch.ComparisonOperator.GREATER_THAN_THRESHOLD,
            threshold = 0,
            evaluation_periods = 1,
            metric = proofpoint.metric_errors(
                period = Duration.minutes(1)
            )
        )

        proofpointalarm.add_alarm_action(
        ...
        event.add_target(
            _targets.LambdaFunction(proofpoint)
        )
