...
class CaretakerJamesBrine(Stack):
    ...
    ### LAMBDA ###
        ...
        logs = _logs.LogGroup(
            self, 'logs',
            log_group_name = '/aws/lambda/'+jamesbrine.function_name,
            retention = _logs.RetentionDays.ONE_DAY,
            removal_policy = RemovalPolicy.DESTROY
        )

        jamesbrinealarm = _cloudwatch.Alarm(
            self, 'jamesbrinealarm',
            comparison_operator = _cloudwatch.ComparisonOperator.GREATER_THAN_THRESHOLD,
            threshold = 0,
            evaluation_periods = 1,
            metric = jamesbrine.metric_errors(
                period = Duration.minutes(1)
            )
        )

        jamesbrinealarm.add_alarm_action(
        ...
        event.add_target(
            _targets.LambdaFunction(jamesbrine)
        )
