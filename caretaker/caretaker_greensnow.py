...
class CaretakerGreenSnow(Stack):
    ...
    ### LAMBDA ###
        ...
        logs = _logs.LogGroup(
            self, 'logs',
            log_group_name = '/aws/lambda/'+greensnow.function_name,
            retention = _logs.RetentionDays.ONE_DAY,
            removal_policy = RemovalPolicy.DESTROY
        )

        greensnowalarm = _cloudwatch.Alarm(
            self, 'greensnowalarm',
            comparison_operator = _cloudwatch.ComparisonOperator.GREATER_THAN_THRESHOLD,
            threshold = 0,
            evaluation_periods = 1,
            metric = greensnow.metric_errors(
                period = Duration.minutes(1)
            )
        )

        greensnowalarm.add_alarm_action(
