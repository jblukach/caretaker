...
class CaretakerDigitalSide(Stack):
    ...
    ### LAMBDA ###
        ...
        logs = _logs.LogGroup(
            self, 'logs',
            log_group_name = '/aws/lambda/'+digitalside.function_name,
            retention = _logs.RetentionDays.ONE_DAY,
            removal_policy = RemovalPolicy.DESTROY
        )

        digitalsidealarm = _cloudwatch.Alarm(
            self, 'digitalsidealarm',
            comparison_operator = _cloudwatch.ComparisonOperator.GREATER_THAN_THRESHOLD,
            threshold = 0,
            evaluation_periods = 1,
            metric = digitalside.metric_errors(
                period = Duration.minutes(1)
            )
        )

        digitalsidealarm.add_alarm_action(
    ...
    ### LAMBDA ###
        ...
        digitalsidedomainalarm = _cloudwatch.Alarm(
            self, 'digitalsidedomainalarm',
            comparison_operator = _cloudwatch.ComparisonOperator.GREATER_THAN_THRESHOLD,
            threshold = 0,
            evaluation_periods = 1,
            metric = digitalsidedomain.metric_errors(
                period = Duration.minutes(1)
            )
        )

        digitalsidedomainalarm.add_alarm_action(
            _actions.SnsAction(topic)
        )
