...
class CaretakerChecksum(Stack):
    ...
    ### LAMBDA ###
        ...
        checksum = _lambda.Function(
            self, 'checksum',
            runtime = _lambda.Runtime.PYTHON_3_12,
            architecture = _lambda.Architecture.ARM_64,
            code = _lambda.Code.from_asset('sources/ip/checksum'),
            timeout = Duration.seconds(900),
            handler = 'checksum.handler',
            environment = dict(
                AWS_ACCOUNT = account,
                FEED_TABLE = 'feed',
                VERIFY_TABLE = 'verify',
        ...
        logs = _logs.LogGroup(
            self, 'logs',
            log_group_name = '/aws/lambda/'+checksum.function_name,
            retention = _logs.RetentionDays.ONE_DAY,
            removal_policy = RemovalPolicy.DESTROY
        )

        checksumalarm = _cloudwatch.Alarm(
            self, 'checksumalarm',
            comparison_operator = _cloudwatch.ComparisonOperator.GREATER_THAN_THRESHOLD,
            threshold = 0,
            evaluation_periods = 1,
            metric = checksum.metric_errors(
                period = Duration.minutes(1)
            )
        )

        checksumalarm.add_alarm_action(
        ...
        event.add_target(
            _targets.LambdaFunction(checksum)
        )