...
class CaretakerBinaryDefense(Stack):
    ...
    ### LAMBDA ###
        ...
        logs = _logs.LogGroup(
            self, 'logs',
            log_group_name = '/aws/lambda/'+binarydefense.function_name,
            retention = _logs.RetentionDays.ONE_MONTH,
            removal_policy = RemovalPolicy.DESTROY
        )

        binarydefensealarm = _cloudwatch.Alarm(
            self, 'binarydefensealarm',
            comparison_operator = _cloudwatch.ComparisonOperator.GREATER_THAN_THRESHOLD,
            threshold = 0,
            evaluation_periods = 1,
            metric = binarydefense.metric_errors(
                period = Duration.minutes(1)
            )
        )

        binarydefensealarm.add_alarm_action(
        ...
        event.add_target(
            _targets.LambdaFunction(binarydefense)
        )
