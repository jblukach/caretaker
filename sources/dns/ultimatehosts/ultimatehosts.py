...
def handler(event, context):
    ...
    response = requests.get('https://hosts.ubuntu101.co.za/domains.list', headers=headers)
    data = response.text
    ...
    for match in matches:
        feed.put_item(
            Item = {
                'pk': 'DNS#',
                'sk': 'DNS#'+str(match)+'#SOURCE#github.com/Ultimate-Hosts-Blacklist',
                'dns': str(match),
