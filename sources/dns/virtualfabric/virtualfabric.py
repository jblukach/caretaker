...
def handler(event, context):
    ...
    response = requests.get('https://dl.threat-list.com/0/domains.txt', headers=headers)
    data = response.text
    ...
    for match in matches:
        feed.put_item(
            Item = {
                'pk': 'DNS#',
                'sk': 'DNS#'+str(match)+'#SOURCE#virtualfabric.com',
                'dns': str(match),
    ...
    return {
        'statusCode': 200,
        'body': json.dumps('Check Virtual Fabric Blocklist')
    }