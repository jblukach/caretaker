...
def handler(event, context):
    ...
    response = requests.get('https://phishstats.info/phish_score.csv', headers=headers)
    data = response.text
    ...
    for match in matches:
        feed.put_item(
            Item = {
                'pk': 'DNS#',
                'sk': 'DNS#'+str(match)+'#SOURCE#phishstats.info',
                'dns': str(match),
    ...
    return {
        'statusCode': 200,
        'body': json.dumps('Check Phish Stats')
    }