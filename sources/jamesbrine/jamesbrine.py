...
def handler(event, context):
    ...
    response = requests.get('https://jamesbrine.com.au/iplist.txt')
    data = response.text
    ...
    for match in matches:
        feed.put_item(
            Item = {
                'pk': 'IP#',
                'sk': 'IP#'+str(match)+'#SOURCE#jamesbrine.com.au',
                'ip': str(match),
                'source': 'jamesbrine.com.au',
                'last': seen,
                'epoch': epoch
            }
        )
    ...
    return {
        'statusCode': 200,
        'body': json.dumps('Check James Brine Blocklist')
    }