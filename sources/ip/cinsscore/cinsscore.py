...
def handler(event, context):
    ...
    response = requests.get('http://cinsscore.com/list/ci-badguys.txt', headers=headers)
    data = response.text
    ...
    f = open('/tmp/cinsscore4.txt', 'w')
    g = open('/tmp/cinsscore6.txt', 'w')
        ...
        if ipaddress.ip_network(line).version == 4:
            ...
            if len(results) > 0:
                feed.put_item(
                    Item = {
                        'pk': 'IP#',
                        'sk': 'IP#'+str(line)+'#SOURCE#cinsscore.com',
                        'ip': str(line),
    ...
    for match in matches:
        feed.put_item(
            Item = {
                'pk': 'IP#',
                'sk': 'IP#'+str(match)+'#SOURCE#cinsscore.com',
                'ip': str(match),
                'source': 'cinsscore.com',
                'last': seen,
                'epoch': epoch,
    ...
    return {
        'statusCode': 200,
        'body': json.dumps('Check CINS Score Reputation')
    }