...
def handler(event, context):
    ...
    response = requests.get('https://raw.githubusercontent.com/securityscorecard/SSC-Threat-Intel-IoCs/master/KillNet-DDoS-Blocklist/ipblocklist.txt', headers=headers)
    data = response.text
    ...
    for match in matches:
        feed.put_item(
            Item = {
                'pk': 'IP#',
                'sk': 'IP#'+str(match)+'#SOURCE#securityscorecard.com',
                'ip': str(match),
    ...
    return {
        'statusCode': 200,
        'body': json.dumps('Check Security Score Card Blocklist')
    }