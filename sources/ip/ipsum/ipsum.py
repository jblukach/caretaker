...
def handler(event, context):
    ...
    response = requests.get('https://raw.githubusercontent.com/stamparm/ipsum/master/ipsum.txt', headers=headers)
    data = response.text
    ...
    for match in matches:
        feed.put_item(
            Item = {
                'pk': 'IP#',
                'sk': 'IP#'+str(match)+'#SOURCE#github.com/stamparm',
                'ip': str(match),
    ...
    return {
        'statusCode': 200,
        'body': json.dumps('Check IPSum Blocklist')
    }