...
def handler(event, context):
    ...
    response = requests.get('https://rules.emergingthreats.net/blockrules/compromised-ips.txt', headers=headers)
    data = response.text
    ...
    f = open('/tmp/proofpoint4.txt', 'w')
    g = open('/tmp/proofpoint6.txt', 'w')
        ...
        if ipaddress.ip_network(line).version == 4:
            ...
            if len(results) > 0:
                feed.put_item(
                    Item = {
                        'pk': 'IP#',
                        'sk': 'IP#'+str(line)+'#SOURCE#emergingthreats.net',
                        'ip': str(line),
    ...
    for match in matches:
        feed.put_item(
            Item = {
                'pk': 'IP#',
                'sk': 'IP#'+str(match)+'#SOURCE#emergingthreats.net',
                'ip': str(match),
                'source': 'emergingthreats.net',
                'last': seen,
                'epoch': epoch,
    ...
    return {
        'statusCode': 200,
        'body': json.dumps('Check Emerging Threats Reputation')
    }