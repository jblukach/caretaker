...
def handler(event, context):
    ...
    response = requests.get('https://hosts.ubuntu101.co.za/ips.list', headers=headers)
    data = response.text
    ...
    f = open('/tmp/ultimatehosts4.txt', 'w')
    g = open('/tmp/ultimatehosts6.txt', 'w')
        ...
        if ipaddress.ip_network(line).version == 4:
            ...
            if len(results) > 0:
                feed.put_item(
                    Item = {
                        'pk': 'IP#',
                        'sk': 'IP#'+str(line)+'#SOURCE#github.com/Ultimate-Hosts-Blacklist',
                        'ip': str(line),
                        'source': 'github.com/Ultimate-Hosts-Blacklist',
                        'last': seen,
                        'epoch': epoch,
                        'ttl': ttl
                    }
    ...
    s3.meta.client.upload_file(
        '/tmp/ultimatehosts4.txt',
        os.environ['S3_BUCKET'],
        'ipv4/ultimatehosts.txt',
        ExtraArgs = {
    ...
    s3.meta.client.upload_file(
        '/tmp/ultimatehosts6.txt',
        os.environ['S3_BUCKET'],
        'ipv6/ultimatehosts.txt',
        ExtraArgs = {
            'ContentType': "text/plain"
        }
    )
    ...
    for match in matches:
        feed.put_item(
            Item = {
                'pk': 'IP#',
                'sk': 'IP#'+str(match)+'#SOURCE#github.com/Ultimate-Hosts-Blacklist',
                'ip': str(match),
                'source': 'github.com/Ultimate-Hosts-Blacklist',
                'last': seen,
                'epoch': epoch,
    ...
    return {
        'statusCode': 200,
        'body': json.dumps('Check Ultimate Hosts')
    }