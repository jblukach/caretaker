...
def handler(event, context):
    ...
    response = requests.get('https://dl.threat-list.com/ips/ips.txt', headers=headers)
    data = response.text
    ...
    f = open('/tmp/virtualfabric4.txt', 'w')
    g = open('/tmp/virtualfabric6.txt', 'w')
        ...
        if line.startswith('#'):
            ...
            if len(results) > 0:
                feed.put_item(
                    Item = {
                        'pk': 'IP#',
                        'sk': 'IP#'+str(line)+'#SOURCE#virtualfabric.com',
                        'ip': str(line),
    ...
    s3.meta.client.upload_file(
        '/tmp/virtualfabric4.txt',
        'projectcaretaker',
        'ipv4/virtualfabric.txt',
        ExtraArgs = {
    ...
    s3.meta.client.upload_file(
        '/tmp/virtualfabric6.txt',
        'projectcaretaker',
        'ipv6/virtualfabric.txt',
        ExtraArgs = {
            'ContentType': "text/plain"
        }
    )
    ...
    for match in matches:
        feed.put_item(
            Item = {
                'pk': 'IP#',
                'sk': 'IP#'+str(match)+'#SOURCE#virtualfabric.com',
                'ip': str(match),
                'source': 'virtualfabric.com',
                'last': seen,
                'epoch': epoch,
    ...
    return {
        'statusCode': 200,
        'body': json.dumps('Check Virtual Fabric Reputation')
    }