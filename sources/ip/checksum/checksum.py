...
def handler(event, context):
    ...
    response = requests.get('https://check-the-sum.fr/api?query=all_ip', headers=headers)
    data = response.json()
    ...
    f = open('/tmp/checksum4.txt', 'w')
    g = open('/tmp/checksum6.txt', 'w')
            ...
            if ipaddress.ip_network(line).version == 4:
                ...
                if len(results) > 0:
                    feed.put_item(
                        Item = {
                            'pk': 'IP#',
                            'sk': 'IP#'+str(line)+'#SOURCE#check-the-sum.fr',
                            'ip': str(line),
                            'source': 'check-the-sum.fr',
                            'last': seen,
                            'epoch': epoch,
                            'ttl': ttl
                        }
    ...
    for match in matches:
        feed.put_item(
            Item = {
                'pk': 'IP#',
                'sk': 'IP#'+str(match)+'#SOURCE#check-the-sum.fr',
                'ip': str(match),
                'source': 'check-the-sum.fr',
                'last': seen,
                'epoch': epoch,
    ...
    return {
        'statusCode': 200,
        'body': json.dumps('Check-The-Sum Reputation')
    }