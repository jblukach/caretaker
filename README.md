...
Every Monday at 9 AM UTC, the Internet addresses for the Broadband Association of North Dakota (BAND) members, North Dakota Univesity System (NDUS), and the State of North Dakota (ITD) get updated from Security Trails, a Recorded Future company.
...
Every Monday at 9 AM UTC, queries run against **Censys** for certificates with the province of either **North Dakota** or the abbreviation **ND** to collect **21,746** domain names for ```apex``` and ```www``` brand monitoring.
...
Every Monday at 9:25 AM UTC, the domain names from the certificates are normalized to guarantee valid top-level domains with wild cards removed.
...
Every day starting at 9:20 AM UTC, domains are checked for **MX** records to determine if an email server is present; once identified, verification of **DMARC** and **SPF** records occurs.
