...
def handler(event, context):
    ...
    print('Downloaded ' + str(len(responsedata)) + ' IPv6 cidrs')

    for item in responsedata:

        db.execute('INSERT INTO distillery (cidr, firstip, lastip) VALUES (?, ?, ?)', (item['cidr'], str(item['firstip']), str(item['lastip'])))
